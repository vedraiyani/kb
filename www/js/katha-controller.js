angular.module('starter.katha-controller', [])

        .controller('KathaCtrl', function($scope, $ionicLoading, SearchSongs) {
            ...
            if (cordovaEvents.isOnline) {
                SearchSongs.get('genre', 'katha', 15 ,'', function(songs) {
                    $scope.songs = songs;
                    $scope.hideLoading();
                });
            } else {
                SearchSongs.getDB('katha', function(rows) {
